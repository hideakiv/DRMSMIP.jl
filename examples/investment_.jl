...
function generate_sample(π::Array{Float64})::Array{DRMSMIP.Sample}
    ...
    for ii in 1:Np
        ...
        ret[ii] = DRMSMIP.Sample(Dict{Symbol, Union{Float64,<:AbstractArray{Float64}}}(:π => π_samp), 1/Np)
    end
    return ret
end
...
function create_nodes()::DD.Tree
    ...
    set = DRMSMIP.WassersteinSet(ξ_samp, 1.0, DRMSMIP.norm_L1)
    cost = Dict{Any, Float64}()
    tree = DD.Tree(ξ, set, cost)
...
function create_nodes!(tree::DD.Tree, pt::Int)
    for scenario = 1:2^L
        ...
        if DD.get_stage(tree, pt) != K-1
            ...
            set = DRMSMIP.WassersteinSet(ξ_samp, 1.0, DRMSMIP.norm_L1)
            cost = Dict{Any, Float64}()
            id = DD.add_child!(tree, pt, ξ, set, cost)
        else 
            cost = Dict{Any, Float64}()
...
det = DRMSMIP.create_Wasserstein_deterministic!(tree)
...
algo = DRMSMIP.DR_LagrangeDual(tree)
